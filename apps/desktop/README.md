...
A build that carried one also registers the bundled `ppt-master` presentation skill. Preparation copies it to `runtime/ppt-skills` outside ASAR from the checkout named by `DSH_DESKTOP_PPT_SKILL_DIR`, defaulting to a `ppt-master` checkout beside this repository, and drops interpreter bytecode and caches. The Host reads it from read-only resources, so it depends on no user-home state and an upgrade replaces it in place; a build or installation without the payload logs a warning and omits the skill rather than failing startup. The skill's Python dependencies are not part of the bundled distributions: routes that need PyYAML, skia-pathops, uharfbuzz, PyMuPDF, EbookLib, or edge-tts report a missing package until the interpreter in use provides it.
